...
## Build Order (6 hour plan)

### Hour 0-1: Foundation
- [ ] Scaffold Vite + React + Tailwind
- [ ] Set up Express server with `/api/assess` stub that returns mock data
- [ ] Build IntakeForm component with all 6 sections
- [ ] Verify form state management works end-to-end

### Hour 1-2: Program logic
- [ ] Write `programs.js` with full program registry (all 15 programs, eligibility criteria, URLs, docs)
- [ ] Write `prompt.js` system prompt builder with cascade rules and negative interactions
- [ ] Connect Express route to Anthropic API with web search enabled
- [ ] Test with 2-3 household profiles, verify JSON response parses correctly

### Hour 2-3: Results UI
- [ ] Build ResultsView, SummaryBar, ProgramCard components
- [ ] Implement card sorting (auto-enrolled → likely → possible)
- [ ] Build CascadeConnector visual between linked cards
- [ ] Add loading skeleton

### Hour 3-4: Polish & edge cases
- [ ] Add Spanish translations (en.json, es.json)
- [ ] Build LanguageToggle, wire i18n through all components
- [ ] Handle API errors gracefully (timeout, rate limit, malformed response)
- [ ] Test edge cases: household with no eligible programs, all programs eligible, mixed eligibility

### Hour 4-5: Prompt refinement
- [ ] Run 5+ test profiles through the system, compare output against manual eligibility checks
- [ ] Tune system prompt: fix any programs being missed or incorrectly flagged
- [ ] Adjust dollar estimates for accuracy
- [ ] Add caseworker tips to prompt for high-value programs

### Hour 5-6: Demo prep
- [ ] Set up demo profile (family of 4, Bronx, $38k, SNAP, Con Edison)
- [ ] Verify demo flow works end-to-end without errors
- [ ] Deploy to Vercel/Render or confirm local demo setup
- [ ] Build presentation deck (Billy's responsibility)
