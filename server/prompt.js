...
{
  "executiveSummary": "REQUIRED. 2-3 prose paragraphs in plain language. Paragraph 1: The single best first step and the exact reason why (name the program, explain the cascade it triggers, give the dollar impact). Paragraph 2: The full recommended application order by priority, with brief reasoning for each step and key deadlines to hit. Paragraph 3: Document prep advice — what to gather first and why it unlocks multiple programs at once.",
  "programs": [
    {
      "programId": "heap_regular | heap_emergency | heap_cooling | heap_clean_tune | heap_herr | liheap | snap | medicaid | coned_eal | eap_national_grid | nyserda_weatherization | nyserda_empower | hra_home_energy | solar_for_all | energyshare | utility_arrears | lifeline",
      "name": "Full program name",
      "description": "REQUIRED. 1-2 sentences describing exactly what this program provides — be concrete (dollar amount, what it covers, how it is delivered, whether it is one-time or annual).",
      "qualifyingFactors": ["Short specific reason this household qualifies — name the exact rule (e.g. 'SSI enrollment = categorical eligibility, no income check required')"],
      "disqualifyingFactors": ["Short specific barrier or concern — be concrete. Use [] if none."],
      "qualifies": true,
      "confidenceLevel": "high | medium | low",
      "alreadyEnrolled": false,
      "eligibility": "likely | possible | unlikely",
      "reason": "One sentence summary.",
      "estimatedValue": "$500–$700/yr",
      "estimatedAnnualBenefit": 600,
      "applicationUrl": "Current URL from the portal search you performed at the start, or null if not found.",
      "requiredDocuments": ["document 1", "document 2"],
      "notes": "One short sentence tip or null."
    }
  ]
}

Rules:
- Include ALL 17 programs (heap_regular, heap_emergency, heap_cooling, heap_clean_tune, heap_herr, liheap, snap, medicaid, coned_eal, eap_national_grid, nyserda_weatherization, nyserda_empower, hra_home_energy, solar_for_all, energyshare, utility_arrears, lifeline). Never skip a program.
- Mirror profile existingBenefits in alreadyEnrolled fields.
- Keep reason, notes, and each bullet point short (one sentence max).
- qualifyingFactors: 1-4 bullets. disqualifyingFactors: 0-3 bullets, empty array if none.
`;
...
13. utility_arrears — Utility Arrears Forgiveness (Con Edison / National Grid)
    ...
    Must be current on Con Edison EAL or National Grid EAP.
...
NEGATIVE INTERACTIONS (flag in disqualifyingFactors or notes when relevant):
...
6. Flag negative interactions in disqualifyingFactors or notes fields.
7. Set alreadyEnrolled=true for programs listed in the profile's existingBenefits.
8. At the start, do 2-3 targeted web searches to get current URLs for the main portals: (a) search "access.nyc.gov HEAP energy assistance 2025" for HRA/OTDA programs, (b) search "nyserda.ny.gov EmPower WAP 2025" for NYSERDA programs, (c) search "coned.com energy affordability program 2025" for Con Edison. Use those results for all relevant programs — do NOT search individually for every program.
9. The executiveSummary must be written in plain ${lang} — no jargon, no abbreviations on first use.
